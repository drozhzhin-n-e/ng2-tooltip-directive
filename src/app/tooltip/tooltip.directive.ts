import {Directive, ElementRef, HostListener, Input, ComponentFactoryResolver, EmbeddedViewRef, ApplicationRef, Injector, ComponentRef} from '@angular/core';
...
export class TooltipDirective {
  ...
  @Input('show-delay') set showDelay(value: any) {
    if (value){
  ...
  get showDelay(){
    let result = this.delay || this._showDelay;
  ...
  get hideDelay(){
    if (this.isMobile()){
      return (this._hideDelay >= this.hideDelayMobile) ? this._hideDelay : this.hideDelayMobile;
    } else {
  ...
  onMouseEnter() {
    if (this.trigger != 'hover' || this.isMobile()){
      return;
    }
  ...
  get destroyDelay() {
    return Number(this.hideDelay) + Number(this.animationDuration);
  }
